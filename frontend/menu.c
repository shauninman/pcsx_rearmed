...
static void tmp_key_config_loop(const me_bind_action *opts, int opt_cnt, int player_idx)
{
...
static int key_config_loop_wrap(int id, int keys)
{
	switch (id) {
		case MA_CTRL_PLAYER1:
			tmp_key_config_loop(me_ctrl_actions, array_size(me_ctrl_actions) - 1, 0);
			break;
...
//	mee_enum_h    ("Confirm savestate",        0, dummy, men_confirm_save, h_confirm_save),
	...
	mee_enum      ("Auto frameskip type",      0, pl_rearmed_cbs.frameskip_type, men_fs_type),
